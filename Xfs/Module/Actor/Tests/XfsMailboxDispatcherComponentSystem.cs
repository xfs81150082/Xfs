...
namespace Xfs
{
	...
	public static class MailboxDispatcherComponentHelper
	{
		...
		{
			XfsSceneType appType = XfsStartConfigComponent.Instance.StartConfig.SenceType;

			self.MailboxHandlers.Clear();

			//List<Type> types = XfsEventSystem.Instance.GetTypes(typeof(XfsMailboxHandlerAttribute));
			HashSet<Type> types = XfsEventSystem.Instance.GetTypes(typeof(XfsMailboxHandlerAttribute));
