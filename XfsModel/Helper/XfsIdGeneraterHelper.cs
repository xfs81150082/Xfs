...
namespace Xfs
{
    public static class XfsIdGeneraterHelper
    {
		...
		public static long GenerateId()
		{
			...
			if (++value22 > ushort.MaxValue - 1)
			{
				//Log.Error($"id is not enough! value: {value}");
				Console.WriteLine($"id is not enough! value: {value22}");
			}

			if (time > int.MaxValue)
			{
				//Log.Error($"time > int.MaxValue value: {time}");
				Console.WriteLine($"time > int.MaxValue value: {time}");
			}

			IdStruct idStruct = new IdStruct(process, (uint)time, (ushort)value22);
	...
	//		{
	//			//Log.Error($"id is not enough! value: {value}");
	//			Console.WriteLine($"id is not enough! value: {value}");
